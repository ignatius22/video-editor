...
class BullQueue extends EventEmitter {
  ...
  setupProcessors() {
    ...
    this.queue.process('cropImage', this.CONCURRENCY, async (bullJob) => {
      return this.processCropImage(bullJob);
    });

    // Process image resize jobs
    this.queue.process('resizeImage', this.CONCURRENCY, async (bullJob) => {
      return this.processResizeImage(bullJob);
    });

    // Process image convert jobs
    this.queue.process('convertImage', this.CONCURRENCY, async (bullJob) => {
  ...
  async processCropImage(bullJob) {
    const { imageId, operation_id, width, height, x, y } = bullJob.data;
    ...
    const originalImagePath = `./storage/${image.image_id}/original.${image.extension}`;
    const dimensions = `${width}x${height}_${x}_${y}`;
    const croppedImagePath = `./storage/${image.image_id}/crop_${dimensions}.jpg`;
    ...
    try {
      ...
      // Update operation status to processing
      if (operation_id) {
        await imageService.updateOperationStatus(operation_id, 'processing');
      }

      // Crop image
      await bullJob.progress(25);
      await FF.cropImage(originalImagePath, croppedImagePath, width, height, x, y);
      ...
      // Update operation status to completed
      if (operation_id) {
        await imageService.updateOperationStatus(operation_id, 'completed', croppedImagePath);
      }
      ...
      return JSON.stringify({ width, height, x, y });
    } catch (error) {
      console.error(`❌ Image crop failed:`, error);

      // Update operation status to failed
      if (operation_id) {
        await imageService.updateOperationStatus(operation_id, 'failed', null, error.message);
      }

      util.deleteFile(croppedImagePath);
      throw error;
    }
  }

  async processResizeImage(bullJob) {
    const { imageId, operation_id, width, height } = bullJob.data;
    const imageService = require('../database/services/imageService');
    ...
    const originalImagePath = `./storage/${image.image_id}/original.${image.extension}`;
    const resizedImagePath = `./storage/${image.image_id}/${width}x${height}.jpg`;
    ...
    try {
      ...
      // Resize image
      await bullJob.progress(25);
      await FF.resizeImage(originalImagePath, resizedImagePath, width, height);
      ...
      // Update operation status to completed
      if (operation_id) {
        await imageService.updateOperationStatus(operation_id, 'completed', resizedImagePath);
      }
      ...
      util.deleteFile(resizedImagePath);
      throw error;
    }
  }

  async processConvertImage(bullJob) {
    const { imageId, operation_id, targetFormat, originalFormat } = bullJob.data;
    const imageService = require('../database/services/imageService');
    ...
    const originalImagePath = `./storage/${image.image_id}/original.${image.extension}`;
    const convertedImagePath = `./storage/${image.image_id}/converted.${targetFormat}`;
    ...
    try {
      ...
      await bullJob.progress(25);
      await FF.convertImageFormat(originalImagePath, convertedImagePath, targetFormat);
      ...
      // Update operation status to completed
      if (operation_id) {
        await imageService.updateOperationStatus(operation_id, 'completed', convertedImagePath);
      }

      // Complete
      await bullJob.progress(100);

      console.log(`✅ Image converted: ${imageId} from ${originalFormat} to ${targetFormat}`);

      return JSON.stringify({ targetFormat, originalFormat });
    } catch (error) {
      ...
      util.deleteFile(convertedImagePath);
      throw error;
    }
  }
